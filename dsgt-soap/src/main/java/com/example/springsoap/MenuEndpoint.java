...
public class MenuEndpoint {
    ...
    private OrderConfirmation orderrepo;

    @Autowired
    public MenuEndpoint(MealRepository mealrepo, OrderConfirmation orderrepo) {
    ...
    public AddOrderResponse addOrder(@RequestPayload AddOrderRequest request) {
        ...
        Meal meals = request.getMeals();

        Order order = new Order();
        order.setAddress(address);
        order.setMeals(meals);

        response.setOrder(orderrepo.addNewOrder(order));
